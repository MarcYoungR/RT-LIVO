...
void VIOManager::generateVisualMapPoints(cv::Mat img, vector<pointWithVar> &pg)
{
  ...
  for (int i = 0; i < pg.size(); i++)
  {
    ...
    if (new_frame_->cam_->isInFrame(pc.cast<int>(), border)) // 20px is the patch size in the matcher
    {
      ...
      // ==========================================
      // [新增] 动态掩码检查 (Mask Check)
      // ==========================================
      if (dynamic_mask != nullptr && !dynamic_mask->empty()) 
      {
          int u = static_cast<int>(pc[0]);
          int v = static_cast<int>(pc[1]);

          // 确保坐标在 Mask 范围内
          if (u >= 0 && u < dynamic_mask->cols && v >= 0 && v < dynamic_mask->rows) 
          {
              // 如果 Mask 值为 0 (黑色/动态)，直接跳过该点！
              if (dynamic_mask->at<uchar>(v, u) == 0) 
              {
                  continue; 
              }
          }
